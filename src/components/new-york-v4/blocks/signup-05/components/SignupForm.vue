...
const password = ref('')
const otp = ref('')
const step = ref(1) // 1: Signup info, 2: OTP verification (otp-05)
const error = ref('')
...
const handleVerifyOtp = async () => {
  if (!otp.value || otp.value.length < 6) {
    error.value = 'Please enter the complete 6-digit verification code'
    return
  }
  error.value = ''
  try {
    ...
    if (authStore.user || authStore.session) {
      ...
      error.value = err.message || 'Invalid or expired OTP code'
    }
  }
}
...
<template>
  <div :class="cn('flex flex-col gap-6', props.class)">
    ...
    <form v-if="step === 1" @submit.prevent="handleSignUp">
      <FieldGroup>
        ...
        <div class="text-center text-xs text-muted-foreground mt-2">
          ...
          <router-link to="/auth/login" class="underline underline-offset-4 text-primary font-medium">
            Sign in
          </router-link>
        </div>
      </FieldGroup>
    ...
    <!-- Step 2: OTP Verification Form (otp-05) -->
    <form v-else-if="step === 2" @submit.prevent="handleVerifyOtp">
      <FieldGroup>
        ...
        <div v-if="successMessage" class="text-green-600 dark:text-green-400 text-sm font-normal text-center">
          {{ successMessage }}
        </div>

        <Field>
          <FieldLabel for="otp" class="sr-only"> Verification code </FieldLabel>
          <InputOTP
            id="otp"
            v-model="otp"
            :maxlength="6"
            required
            container-class="gap-4 justify-center"
          >
          ...
          <FieldDescription class="text-center mt-2">
            Didn't receive the code?
            <button
              type="button"
              @click="handleResend"
              :disabled="resending"
              class="underline underline-offset-4 text-primary font-medium hover:text-primary/80 disabled:opacity-50"
            >
